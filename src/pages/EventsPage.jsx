...
const EventsPage = () => {
  ...
  const [filters, setFilters] = useState({ city: '', date: '', time: '' });
...
const SidebarButton = ({ to, label, navigate, disabled = false }) => (
  <button
    onClick={() => {
      if (disabled) {
    ...
    className={`text-left px-4 py-2 rounded-xl transition font-medium ${disabled
      ? 'bg-white/10 cursor-not-allowed text-gray-300'
